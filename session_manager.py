...
class SessionManager:
    def __init__(self):
        ...
        print(
            f"📝 Session Manager Inizializzato (Accent-Insensitive + SQLite: {self.db_path})"
        )
        ...
        try:
            with sqlite3.connect(self.db_path, check_same_thread=False) as conn:
                ...
                cursor.execute(
                    """
                    INSERT INTO songs (
                        id, title, artist, composer, album, timestamp,
                        duration_ms, score, type, isrc, upc
                    )
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
    ...
    def _update_composer_in_db(self, song_id, composer):
        ...
        try:
            with sqlite3.connect(self.db_path, check_same_thread=False) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE songs SET composer = ? WHERE id = ?", (composer, song_id)
                )
                conn.commit()
        except Exception as e:
    ...
    def _normalize_string(self, text):
        ...
        # 2. Rimozione Keyword
        text = re.sub(
            r"(?i)\b(feat\.|ft\.|remix|edit|version|karaoke|live)\b.*", "", text
        )
    ...
    def _are_songs_equivalent(self, new_s, existing_s):
        ...
        if dur_new > MIN_VALID_DURATION and dur_ex > MIN_VALID_DURATION:
            ...
            if diff < tolerance:
                print(
                    f"🔄 Duplicato (Zona Critica {similarity:.2f}): "
                    f"'{tit_new}' == '{tit_ex}' (Diff: {diff}ms)"
                )
                return True
    ...
    def _background_enrichment(self, entry, target_artist):
        ...
        with self.lock:
            target_song = next(
                (s for s in self.playlist if s["id"] == entry["id"]), None
            )
