...
class MetadataManager:
    ...
    def find_composer(
        ...
        artists_to_try = []
        if setlist_artist:
            artists_to_try.append(setlist_artist)
        if detected_artist and detected_artist != setlist_artist:
    ...
    def _search_deezer(self, title, artist):
        try:
            ...
            for res in data.get("data", []):
                ...
                if SequenceMatcher(None, title_norm, found_title).ratio() < 0.6:
                    continue

                found_artist = self._clean_string(res.get("artist", {}).get("name", ""))
                if target_norm not in found_artist and found_artist not in target_norm:
                    continue
    ...
    def _search_mb_by_isrc(self, isrc):
        try:
            res = musicbrainzngs.get_recordings_by_isrc(
                isrc, includes=["work-rels", "artist-rels"]
            )
    ...
    def _get_comp(self, rid):
        try:
            ...
            rec = musicbrainzngs.get_recording_by_id(
                rid, includes=["work-rels", "artist-rels"]
            )
